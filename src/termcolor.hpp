/*
 * Termcolor
 * Written by Ihor Kalnytskyi, Copyright (c) 2013
 *
...
// Detect target's platform and set some macros in order to wrap platform specific code this library depends on.
...
// If implementation has not been explicitly set, try to choose one based on target platform.
...
// These headers provide isatty()/fileno() functions, which are used for testing whether a standard stream refers to the terminal.
#if defined(TERMCOLOR_TARGET_POSIX)
#   include <unistd.h>
#elif defined(TERMCOLOR_TARGET_WINDOWS)
#   include <io.h>
...
namespace termcolor
{
     ...
     * I have to introduce this namespace which is used for internal purpose and should't be access from the user code.
     */
    namespace _internal
    {
        /* An index to be used to access a private storage of I/O streams. See colorize / nocolorize I/O manipulators for details.
         * Due to the fact that static variables ain't shared between translation units,
         * inline function with local static variable is used to do the trick and share the variable value between translation units.
         */
        inline int colorize_index()
        {
        ...
        /* Since C++ hasn't a true way to extract stream handler from the a given `std::ostream` object,
         * I have to write this kind of hack.
         */
        inline
        ...
        /* Since C++ hasn't a true way to extract stream handler from the a given `std::wostream` object,
         * I have to write this kind of hack.
         */
        inline
