/**
 * @file Game.cpp
 * @brief Implementation of Game type functions
 * @author LENCLOS Corentin
 * @version 1.0
 */
...
void initGame(const char *players, Game &game) {
    ...
    game.playerList.currentPlayerID = 0; // First entered player starts
    game.currentWord = NULL;
    initialize(game.playerList, length);
    for (uint i = 0; i < length; ++i) {
        ...
        insert(game.playerList, player, i);
    }
}
...
bool loserExists(const Game &game) {
    for (uint i = 0; i < game.playerNumber; ++i) {
        Player &p = read(game.playerList, i);
        if (p.nQuart == 4) {
            return true;
        }
    }
...
void turn(Game &game) {
    ...
    while (continueTest) {
        Player &currentP = read(game.playerList, game.playerList.currentPlayerID);
        char newChar;
...
void askPreviousPlayer(Game &game) {
    ...
    previous(game.playerList);
    Player &askedP = read(game.playerList, game.playerList.currentPlayerID);
    // Word input
    ...
    if (verification == exists) next(game.playerList);
    // Quarter monkey attribution
    Player &p = read(game.playerList, game.playerList.currentPlayerID);
...
void displayWordV(const wordVerification &verification, const char *word, const Player &player) {
    ...
    switch (verification) {
        case exists:
            ...
            break;
        case wrongStart:
            cout << termcolor::red << word << termcolor::reset << " ne commence pas par les lettres attendues";
            break;
        case doesNotExist:
